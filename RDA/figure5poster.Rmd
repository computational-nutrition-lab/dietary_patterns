...
opts_knit$set(root.dir = "~/Dropbox/dietstudy/")

# create a directory for the figure and set it's resolution/format
opts_chunk$set(echo = TRUE, fig.path = "Figsposter/", dev = c("png", "pdf"), dpi = 300)
...
func <- read.delim("~/Dropbox/dietstudy/data/processed_KEGG/ave_KEGG_modules_no_soy.txt", sep = "\t", row.names = 1)

# limit to just the highly variable functions
func_cv <- read.delim("~/Dropbox/dietstudy/data/processed_KEGG/preprocessed_KEGG_modules_filtered_clr.txt", sep = "\t", row.names= 1)
...
food_beta_un <- read.delim("~/Dropbox/dietstudy/data/processed_food/dhydrt_smry_no_soy_beta/unweighted_unifrac_dhydrt.smry.no.soy.txt", sep = "\t", row.names = 1)


food_beta_w <- read.delim("~/Dropbox/dietstudy/data/processed_food/dhydrt_smry_no_soy_beta/weighted_unifrac_dhydrt.smry.no.soy.txt", sep = "\t", row.names = 1)
...
food_vectors_w$DV1 <- food_vectors_w$DV1
food_vectors_w$DV2 <- food_vectors_w$DV2 
food_vectors_w$DV3 <- food_vectors_w$DV3
food_vectors_w$DV4 <- food_vectors_w$DV4 * -1
food_vectors_w$DV5 <- food_vectors_w$DV5 * -1

# give vectors interpretable names for plotting
colnames(food_vectors_w) <- c("Grains", 
                              "Poultry, eggs, and vegetables", 
                              "Alcohol, cheese, and meat",
                              "Fats and snacks",
                              "Cakes and oils")
...
```{r heatmap, echo = F, fig.width = 4, fig.height = 5, include = F}
...
diet_vec_heat <- ggplot(data = dat.m, aes(x=food_vector, y=taxa, fill=correlation)) + 
  ...
  scale_fill_gradient2(low="#5e3c99", mid="#f7f7f7", high="#e66101", midpoint = 0, limit = c(-0.6,0.6), space = "Lab", name = "Spearman") +
...
```{r food_heatmap, echo = F, fig.width=10, fig.height = 5 , include = F}
...
food_vec_heat <- ggplot(data = dat.m, aes(y=food_vector, x=food, fill=correlation)) + 
  geom_tile(color = "white") +
  ...
  labs(x = "Food Group", y = "Dietary Pattern") +
  theme(strip.text.y = element_text(angle = 0, face = "italic"), 
        strip.background = element_rect(color="grey", fill = "white")) +
  coord_fixed(ratio = 1)
