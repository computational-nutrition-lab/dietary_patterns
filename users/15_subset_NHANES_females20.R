...
# Then select only for females in their 20s.
# Version 1
...
# Select FEmales in their 50s
  ...
  glu_2_fems20s <- subset(glu_2_fems, RIDAGEYR >= 20 & RIDAGEYR <= 29 ) 
  
# Check the dimension of the selected data - 151 rows.
  dim(glu_2_fems20s)

# Ensure the ages of the selected subpopulation are between 50-59.  
  table(glu_2_fems20s$RIDAGEYR)

# Look at the distribution of GLU_index among the selected subpopulation.
  table(glu_2_fems20s$GLU_index)
  
# Save the glu_2_fems20s as a txt file.
  write.table(glu_2_fems20s, "Laboratory_data/QCtotal_d_glu_body_meta_demo_fems20s.txt", 
              sep="\t", row.names = F, quote = F)
  
# ----------------------------------------------------------------------------------------------------------------  
# Look at the BMI frequency of each group.
# This uses lighter colors for the subpopulation.
  fems20s_BMIfreq <- ggplot(data=glu_2_fems20s, aes(x=BMXBMI, group=GLU_index, fill=GLU_index)) +
  ...
  # Save the chart as .pdf.
  ggsave("Laboratory_data/fems20s_BMI_by_GLU_index.pdf", 
         fems20s_BMIfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Body weight
    # Make sure the labels in the legend are correct. 
  fems20s_weightfreq <- ggplot(data=glu_2_fems20s, aes(x=BMXWT, group=GLU_index, fill=GLU_index)) +
    geom_density(adjust=1.5, alpha=.4) + space_axes + no_grid +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    labs(x="Body weight (kg)", y="Density") 
  fems20s_weightfreq
  
  ggsave("Laboratory_data/fems20s_weight_by_GLU_index.pdf", 
         fems20s_weightfreq, device="pdf", width=5.3, height=4.5)

# ----------------------------------------------------------------------------------------------------------------  
# Look at the KCAL frequency of each group.   
# Make sure the labels in the legend are correct. 
  fems20s_KCALfreq <- ggplot(data=glu_2_fems20s, aes(x=KCAL, group=GLU_index, color=GLU_index)) +
  ...
  # Save the chart as .pdf.
  ggsave("Laboratory_data/fems20s_KCAL_by_GLU_index.pdf", 
         fems20s_KCALfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Create a boxplot of KCAL of each GLU_index group.
  fems20s_KCAL <- ggplot(glu_2_fems20s, aes(x=GLU_index, y=KCAL, fill=GLU_index)) +
    geom_boxplot(outlier.shape = NA) + no_grid + space_axes +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    geom_jitter(width=0.3)
  fems20s_KCAL
  
  ggsave("Laboratory_data/fems20s_KCAL_by_GLU_index_box.pdf", 
         fems20s_KCAL, device="pdf", width=5.3, height=4.5)
