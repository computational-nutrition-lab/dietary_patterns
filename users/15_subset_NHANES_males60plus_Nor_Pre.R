...
# Then select only for males in their 60 plus (over) that are Normal or Prediabetic.
# Version 1
# Created on 10/21/2022 by Rie Sadohara
# ===============================================================================================================
...
# Select males in their 50s
  ...
  glu_2_males60over <- subset(glu_2_males, RIDAGEYR >= 60 ) 
  
# Check the dimension of the selected data - 151 rows.
  dim(glu_2_males60over)

# Ensure the ages of the selected subpopulation are between 50-59.  
  table(glu_2_males60over$RIDAGEYR)

# Look at the distribution of GLU_index among the selected subpopulation.
  table(glu_2_males60over$GLU_index)
  
  twoway = table(glu_2_males60over$`RIDAGEYR`, glu_2_males60over$GLU_index)
  head(twoway)
  
  # Select only Normal and Diabetic
  glu_2_males60over_NP <- subset(glu_2_males60over, GLU_index == "Normal" | GLU_index == "Prediabetic" ) 
  dim(glu_2_males60over_NP)
  # Should be 205 rows (people).
  
# Save the glu_2_males60over as a txt file.
  write.table(glu_2_males60over_NP, "Laboratory_data/QCtotal_d_glu_body_meta_demo_males60over_NP.txt", 
              sep="\t", row.names = F, quote = F)
  
# ----------------------------------------------------------------------------------------------------------------  
# Look at the BMI frequency of each group.
# This uses lighter colors for the subpopulation.
  males60over_NP_BMIfreq <- ggplot(data=glu_2_males60over_NP, aes(x=BMXBMI, group=GLU_index, fill=GLU_index)) +
    geom_density(adjust=1.5, alpha=0.4) + space_axes + no_grid +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    labs(x="BMI", y="Density")
  males60over_NP_BMIfreq

  # Save the chart as .pdf.
  ggsave("Laboratory_data/males60over_NP_BMI_by_GLU_index.pdf", 
         males60over_NP_BMIfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Body weight
    # Make sure the labels in the legend are correct. 
  males60over_NP_weightfreq <- ggplot(data=glu_2_males60over_NP, aes(x=BMXWT, group=GLU_index, fill=GLU_index)) +
    geom_density(adjust=1.5, alpha=.4) + space_axes + no_grid +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    labs(x="Body weight (kg)", y="Density") 
  males60over_NP_weightfreq
  
  ggsave("Laboratory_data/males60over_NP_weight_by_GLU_index.pdf", 
         males60over_NP_weightfreq, device="pdf", width=5.3, height=4.5)

# ----------------------------------------------------------------------------------------------------------------  
# Look at the KCAL frequency of each group.   
# Make sure the labels in the legend are correct. 
  males60over_NP_KCALfreq <- ggplot(data=glu_2_males60over_NP, aes(x=KCAL, group=GLU_index, color=GLU_index)) +
    ...
    labs(x="KCAL", y="Density") +
    scale_y_continuous(labels= function(x) format(x, scientific = FALSE))
  males60over_NP_KCALfreq
  
  # Save the chart as .pdf.
  ggsave("Laboratory_data/males60over_NP_KCAL_by_GLU_index.pdf", 
         males60over_NP_KCALfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Create a boxplot of KCAL of each GLU_index group.
  males60over_NP_KCAL <- ggplot(glu_2_males60over_NP, aes(x=GLU_index, y=KCAL, fill=GLU_index)) +
    geom_boxplot(outlier.shape = NA) + no_grid + space_axes +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    geom_jitter(width=0.3)
  males60over_NP_KCAL
  
  ggsave("Laboratory_data/males60over_NP_KCAL_by_GLU_index_box.pdf", 
         males60over_NP_KCAL, device="pdf", width=5.3, height=4.5)
...
# Remove samples (rows) that have missing data in BMXBMI
  df <- glu_2_males60over_NP[complete.cases(glu_2_males60over_NP$BMXBMI), ]
...
# KCAL
  # Remove samples (rows) that have missing data in BMXBMI
  df <- glu_2_males60over_NP[complete.cases(glu_2_males60over_NP$KCAL), ]
