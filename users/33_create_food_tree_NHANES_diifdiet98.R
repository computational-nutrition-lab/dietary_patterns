...
# Make the individuals as a vector.   
  selectedind <- QCtotal_b$SEQN
  
# Load the input file to be filtered.
  all.food.record <- read.table("eg_data/NHANES/Food_D12_FC_cc_f.txt", sep="\t", header=T)
...
#  so that the data will be formatted in a compatible way to create food tree.
  FilterDbByDietRecords(food_database_fn = "eg_data/NHANES1516/processed/NHANESDatabase.txt", 
                        food_records_fn  = "eg_data/NHANES/Food_D12_FC_cc_f_diffdiet98.txt",   # output of FormatFoods.  
                        output_fn =        "eg_data/NHANES/Food_D12_FC_cc_f_diffdiet98_red.txt")
...
# Create food tree with the reduced dataset (only reported foods).   
  MakeFoodTree(nodes_fn="data/Food_tree_data/NodeLabelsMCT.txt", # can use this for now. 
               addl_foods_fn = NULL, 
               num.levels = 2,
               food_database_fn=    "eg_data/NHANES/Food_D12_FC_cc_f_diffdiet98_red.txt",  
               output_tree_fn=      "results/Food_tree_NHANES/Food_D12_FC_cc_f_diffdiet98_red_Lv2.txt", 
...
# --------------------------------------------------------------------------------------------------------------
  ...
  ggsave("results/Food_tree_NHANES/Food_D12_FC_cc_f_diffdiet98_red_Lv5.tif", tree_an_hi_o_rt,  
         device = "tiff", width=5, height=5, dpi=200) 
