...
# Ensure the ages of the selected subpopulation are between 50-59.  
  ...
  table(glu_2_males60to79$LBXGLU)

# Make a new vector GLU_index2
  glu_2_males60to79$GLU_index2 <- NA
  
  glu_2_males60to79$LBXGLU <- as.numeric(glu_2_males60to79$LBXGLU)
  
# Add four levels of GLU_index.
  for(i in 1: nrow(glu_2_males60to79)){
    if(     glu_2_males60to79$LBXGLU[i] < 100){ glu_2_males60to79$GLU_index2[i] <- "Normal" }
    else if(glu_2_males60to79$LBXGLU[i] < 110){ glu_2_males60to79$GLU_index2[i] <- "Prediabetic_L" }
    else if(glu_2_males60to79$LBXGLU[i] < 126){ glu_2_males60to79$GLU_index2[i] <- "Prediabetic_H" }
    else{                                         glu_2_males60to79$GLU_index2[i] <- "Diabetic" }
  }
  
  table(glu_2_males60to79$GLU_index2)
  
  median(subset(glu, LBXGLU > 101 & LBXGLU < 126)[, "LBXGLU"])
...
# Save the glu_2_males60to79 as a txt file.
  write.table(glu_2_males60to79, "Laboratory_data/QCtotal_d_glu_body_meta_demo_males60to79.txt", 
              sep="\t", row.names = F, quote = F)
  
# ----------------------------------------------------------------------------------------------------------------  
# Look at the BMI frequency of each group.
# This uses lighter colors for the subpopulation.
  males60to79_BMIfreq <- ggplot(data=glu_2_males60to79, aes(x=BMXBMI, group=GLU_index, fill=GLU_index)) +
  ...
  # Save the chart as .pdf.
  ggsave("Laboratory_data/males60to79_BMI_by_GLU_index.pdf", 
         males60to79_BMIfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Body weight
    # Make sure the labels in the legend are correct. 
  males60to79_weightfreq <- ggplot(data=glu_2_males60to79, aes(x=BMXWT, group=GLU_index, fill=GLU_index)) +
    geom_density(adjust=1.5, alpha=.4) + space_axes + no_grid +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    labs(x="Body weight (kg)", y="Density") 
  males60to79_weightfreq
  
  ggsave("Laboratory_data/males60to79_weight_by_GLU_index.pdf", 
         males60to79_weightfreq, device="pdf", width=5.3, height=4.5)

# ----------------------------------------------------------------------------------------------------------------  
# Look at the KCAL frequency of each group.   
# Make sure the labels in the legend are correct. 
  males60to79_KCALfreq <- ggplot(data=glu_2_males60to79, aes(x=KCAL, group=GLU_index, color=GLU_index)) +
  ...
  # Save the chart as .pdf.
  ggsave("Laboratory_data/males60to79_KCAL_by_GLU_index.pdf", 
         males60to79_KCALfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Create a boxplot of KCAL of each GLU_index group.
  males60to79_KCAL <- ggplot(glu_2_males60to79, aes(x=GLU_index, y=KCAL, fill=GLU_index)) +
    geom_boxplot(outlier.shape = NA) + no_grid + space_axes +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    geom_jitter(width=0.3)
  males60to79_KCAL
  
  ggsave("Laboratory_data/males60to79_KCAL_by_GLU_index_box.pdf", 
