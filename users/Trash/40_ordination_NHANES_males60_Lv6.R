...
# Add metadata (GLU_index) before making a phyloseq object. -- line 126 to do ordination gives an error... because of food level 6?? 
# Version 1
...
# Food
  ...
  food <- read.delim("Foodtree/Food_D12_FC_cc_f_males60s_red_Lv6.dhydrt.otu.txt", row.names=1)
...
# Taxonomy (tax)
  ...
  tax <- read.delim("Foodtree/Food_D12_FC_cc_f_males60s_red_Lv6.taxonomy.txt")
...
# Food tree
  ...
  foodtree <- read_tree("Foodtree/Food_D12_FC_cc_f_males60s_red_Lv6.nwk")
...
# Perform Principal Coordinate Analysis (PCoA) with WEIGHTED unifrac distance of your food data.
  ...
  ordinated_w <- phyloseq::ordinate(phyfoods, method="PCoA", distance="unifrac", weighted=TRUE)  # GIVES ERROR...
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_w, 
        output.fn="Food_D12_FC_cc_f_males60s_red_Lv6_ord_WEIGHTED_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_w, number.of.axes= 10, meta.data= demog_glu, 
                              output.fn= "Food_D12_FC_cc_f_males60s_red_Lv6_ord_WEIGHTED_meta_users.txt")

# Load the output again for plotting.
  loaded_glu_w <- read.table("Food_D12_FC_cc_f_males60s_red_Lv6_ord_WEIGHTED_meta_users.txt",
                             sep="\t", header=T)
...
# Save p1 as a PDF. 
  ggsave("Food_D12_FC_cc_f_males60s_red_Lv6_ord_WEIGHTED_Axis12_p1.pdf", 
...
# Save ellipses as a PDF. 
  ggsave("Food_D12_FC_cc_f_males60s_red_Lv6_ord_WEIGHTED_Axis12_ellipses.pdf", 
...
# Use dispr to do a permutation test for homogeneity of multivariate dispersion
  ...
  # p ~ 0.09. Borderline, so increase the permutations.
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_u, 
        output.fn="Food_D12_FC_cc_f_males60s_red_Lv6_ord_UNweighted_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_u, number.of.axes= 10, meta.data= demog_glu, 
                              output.fn= "Food_D12_FC_cc_f_males60s_red_Lv6_ord_UNweighted_meta_users.txt")
  
# Load the output again for plotting.
  loaded_glu_u <- read.table("Food_D12_FC_cc_f_males60s_red_Lv6_ord_UNweighted_meta_users.txt",
                             sep="\t", header=T)  
...
# Save p1 as a PDF. 
  ggsave("Food_D12_FC_cc_f_males60s_red_Lv6_ord_UNweighted_Axis12_p1.pdf", 
...
# Save ellipses as a PDF. 
  ggsave("Food_D12_FC_cc_f_males60s_red_Lv6_ord_UNweighted_Axis12_ellipses.pdf", 
...
# Use dispr to do a permutation test for homogeneity of multivariate dispersion.
  ...
  # The results will be slightly different because this is a permutation-based test, 
  # which conducts 999 permutation tests by default.  
  # p-values are ~0.5, so, not significant. That means dispersion of the groups is not different.
...
# Generate and save an WEIGHTED unifrac distance matrix of "Samples". 
  WeightedUnifracDis(input.phyloseq.obj = phyfoods, 
                     output.fn = "Food_D12_FC_cc_f_males60s_red_Lv6_ord_WEIGHTED_uni_dis.txt")        

# ---------------------------------------------------------------------------------------------------------------
# Generate and save an UNweighted unifrac distance matrix of "Samples". 
  UnweightedUnifracDis(input.phyloseq.obj = phyfoods, 
                       output.fn = "Food_D12_FC_cc_f_males60s_red_Lv6_ord_UNweighted_uni_dis.txt")        
