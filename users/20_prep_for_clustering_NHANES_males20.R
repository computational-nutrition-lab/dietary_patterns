# ===============================================================================================================
# Prepare NHANES males in their 20s data for PCA and other cluster analysis.
# Version 1
...
# Are BMI and body weight correlated? - Yes.
  plot(glu_3_males20s$BMXBMI, glu_3_males20s$BMXWT)
  cor.test(glu_3_males20s$BMXBMI, glu_3_males20s$BMXWT)

# Define which columns to drop.
  drops <- c("KCAL","GRMS", "MOIS", "NoOfItems")

# Take only the columns whose names are NOT in the drop vector. 
  glu_3_males20s_2 <- glu_3_males20s[ , !(names(glu_3_males20s) %in% drops)]
...
# Take  start.col="PROT" through end.col="P226" plus, "BMXBMI" and "BMXWT".
  BMI_col   <- match("BMXBMI" , names(glu_3_males20s_2)) 
  WT_col    <- match("BMXWT"  , names(glu_3_males20s_2)) 
  start_col <- match("PROT"   , names(glu_3_males20s_2))  
  end_col   <- match("P226"   , names(glu_3_males20s_2)) 
  
# Pick up BMI, weight, and nutrient variables.
  subsetted <- glu_3_males20s_2[ , c(BMI_col, WT_col, start_col:end_col)]
...
# Save the variables after removing correlated variables
  write.table(selected_variables,
              "males20s_QCtotal_d_glu_body_meta_demo_Nut_rv.txt", 
...
# cc is the correlation matrix produced when variables are collapsed by correlation. 
  SaveCorrMatrix(x=cc,
                 out.fn= "males20s_QCtotal_d_glu_body_meta_demo_Nut_corr_mat.txt")
...
# The output is a df called "subsetted".
  BMI_col   <- match("BMXBMI"  , names(glu_3_males20s_2)) 
  WT_col    <- match("BMXWT"   , names(glu_3_males20s_2)) 
  start_col <- match("F_CITMLB", names(glu_3_males20s_2))  
  end_col   <- match("A_DRINKS", names(glu_3_males20s_2))   
...
# Save the variables after removing correlated variables
  write.table(selected_variables,
              "males20s_QCtotal_d_glu_body_meta_demo_Cat_rv.txt", 
              sep="\t", row.names=F, quote=F)
...
# cc is the correlation matrix produced when variables are collapsed by correlation. 
  SaveCorrMatrix(x=cc,
                 out.fn = "males20s_QCtotal_d_glu_body_meta_demo_Cat_corr_mat.txt")
