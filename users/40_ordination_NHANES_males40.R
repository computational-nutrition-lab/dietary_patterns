...
# Food
  ...
  food <- read.delim("Foodtree/Food_D12_FC_cc_f_males40s_red_Lv4.dhydrt.otu.txt", row.names=1)
...
# Taxonomy (tax)
  ...
  tax <- read.delim("Foodtree/Food_D12_FC_cc_f_males40s_red_Lv4.taxonomy.txt")
...
# Food tree
  ...
  foodtree <- read_tree("Foodtree/Food_D12_FC_cc_f_males40s_red_Lv4.nwk")
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_w, 
        output.fn="Food_D12_FC_cc_f_males40s_red_Lv4_ord_WEIGHTED_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_w, number.of.axes= 10, meta.data= demog_glu, 
                              output.fn= "Food_D12_FC_cc_f_males40s_red_Lv4_ord_WEIGHTED_meta_users.txt")

# Load the output again for plotting.
  loaded_glu_w <- read.table("Food_D12_FC_cc_f_males40s_red_Lv4_ord_WEIGHTED_meta_users.txt",
                             sep="\t", header=T)
...
# Save p1 as a PDF. 
  ggsave("Food_D12_FC_cc_f_males40s_red_Lv4_ord_WEIGHTED_Axis12_p1.pdf", 
...
# Save ellipses as a PDF. 
  ggsave("Food_D12_FC_cc_f_males40s_red_Lv4_ord_WEIGHTED_Axis12_ellipses.pdf", 
...
# Use dispr to do a permutation test for homogeneity of multivariate dispersion
  ...
  # p-values are ~0.85, so, not significant. That means the dispersion of each group 
...
# Save the percent variance explained as a txt file.
  Eigen(eigen.input = eigen_percent_u, 
        output.fn="Food_D12_FC_cc_f_males40s_red_Lv4_ord_UNweighted_eigen.txt")
...
# The merged dataframe, 'meta_usersdf', will be used for plotting.
  MergeAxesAndMetadata_NHANES(ord.object= ordinated_u, number.of.axes= 10, meta.data= demog_glu, 
                              output.fn= "Food_D12_FC_cc_f_males40s_red_Lv4_ord_UNweighted_meta_users.txt")
  
# Load the output again for plotting.
  loaded_glu_u <- read.table("Food_D12_FC_cc_f_males40s_red_Lv4_ord_UNweighted_meta_users.txt",
                             sep="\t", header=T)  
...
# Save p1 as a PDF. 
  ggsave("Food_D12_FC_cc_f_males40s_red_Lv4_ord_UNweighted_Axis12_p1.pdf", 
...
# Save ellipses as a PDF. 
  ggsave("Food_D12_FC_cc_f_males40s_red_Lv4_ord_UNweighted_Axis12_ellipses.pdf", 
...
# Use dispr to do a permutation test for homogeneity of multivariate dispersion.
  ...
  # The results will be slightly different because this is a permutation-based test, 
  # which conducts 999 permutation tests by default.  
  # p-value is 0.001, so, significant. That means dispersion of the groups is different.
  # Thus, the assumption of homogeneity of dispersion is NOT met. hmmm..   
  # If it is significant, add pairwise=T argument and see which combination(s) are 
  # significantly different.
  ...
  # Diabetic                  1.0000e-03       0.001
  # Normal      1.2007e-04                     0.011
  # Prediabetic 1.0440e-09    8.6451e-03   
  
# Use adonis to test whether there is a difference between groups' composition. 
  # i.e., composition among groups (food they consumed) is similar or not.
  vegan::adonis(dist_matrix_u ~ phyloseq::sample_data(phyfoods)$GLU_index) 
  # p ~ 0.46. They are not different.  
...
# Generate and save an WEIGHTED unifrac distance matrix of "Samples". 
  WeightedUnifracDis(input.phyloseq.obj = phyfoods, 
                     output.fn = "Food_D12_FC_cc_f_males40s_red_Lv4_ord_WEIGHTED_uni_dis.txt")        

# ---------------------------------------------------------------------------------------------------------------
# Generate and save an UNweighted unifrac distance matrix of "Samples". 
  UnweightedUnifracDis(input.phyloseq.obj = phyfoods, 
                       output.fn = "Food_D12_FC_cc_f_males40s_red_Lv4_ord_UNweighted_uni_dis.txt")        
