...
# Create temporary dataframes for procesing.
  ...
  dim(dfB2)
  head(dfB2)
  table(dfB2$Food_code)
  
  tabB <- as.matrix(dfB2[,-1])                           # convert to matrix, leave out food code
  rownames(tabB) <- dfB2$Food_code
  
  # safety check: food codes match
  identical(dim(tabA), dim(tabB))
  identical(gsub("_.*", "", rownames(tabA)), rownames(tabB))
  identical(colnames(tabA), colnames(tabB))
  ...
  foodcode_grams_df <- data.frame(Food_code = gsub("_.*", "", rownames(out)),
                                  DR1IGRMS = as.numeric(gsub(".*_","", rownames(out))))
...
# Join Food_Dx and result_v.
  ...
  write.table(Food_D1_FC, "eg_data/NHANES/FPED/Food_D1_FC_bymm.txt", sep="\t", row.names=F, quote=F)
  
  #
  
  
    # Check if the results are the same. ############################################################
  food1byloop <- read.table("eg_data/NHANES/FPED/Food_D1_FC.txt", sep="\t", header=T)
  food1bymm <- read.table("eg_data/NHANES/FPED/Food_D1_FC_bymm.txt", sep="\t", header=T)
  dim(food1bymm)
  dim(food1byloop)
  colnames(food1byloop)
  head(food1byloop[, c(1,20, 91:127)])
  head(food1bymm)
  identical(as.numeric(food1byloop$D_TOTAL), as.numeric(food1bymm$D_TOTAL))
  food1byloop$diff = round(as.numeric(food1byloop$PF_MEAT) - as.numeric(food1bymm$PF_MEAT), 5)
  table(food1byloop$diff)
  # GREAT
  
# Matrix Multiplication!!!! D2. ----------------------------------------------------------------------------------------------
  ...
  # reformatting to get a clean table
  foodcode_grams_df <- data.frame(Food_code = gsub("_.*", "", rownames(out)),
                                  DR2IGRMS = as.numeric(gsub(".*_","", rownames(out))))
  ...
  result_2 <- out_df[, !(names(out_df) %in% dropcol)]
  head(result_2)
  
  aaa= as.numeric(out_df$Food_code) - as.numeric(Food_D2$Food_code)
  table(aaa)
  identical(as.numeric(out_df$DR2IGRMS), as.numeric(Food_D2$DR2IGRMS))
  
  # Join Food_Dx and result_v.
  Food_D2_FC = cbind(Food_D2, result_2)
  head(Food_D2_FC, 1)
  
  # Save as a txt file. **** WILL TAKE A FEW MINUTES. *****
  write.table(Food_D2_FC, "eg_data/NHANES/FPED/Food_D2_FC_bymm.txt", sep="\t", row.names=F, quote=F)
  
###### CHECK ##############################################################  
  food2byloop <- read.table("eg_data/NHANES/FPED/Food_D2_FC_byloop.txt", sep="\t", header=T)
  food2bymm <- read.table("eg_data/NHANES/FPED/Food_D2_FC_bymm.txt", sep="\t", header=T)
  dim(food2bymm)
  dim(food2byloop)
  colnames(food2byloop)
  colnames(food2bymm)
  # head(food2byloop[, c(1,20, 91:127)])
  head(food2bymm, 2)
  identical(as.numeric(food2byloop$D_TOTAL), as.numeric(food2bymm$D_TOTAL))
  food2byloop$diff = round(as.numeric(food2byloop$D_TOTAL) - as.numeric(food2bymm$D_TOTAL), 5)
  food2byloop$diff = round(as.numeric(food2byloop$PF_MEAT) - as.numeric(food2bymm$PF_MEAT), 5)
  food2byloop$diff = as.numeric(food2byloop$Food_code) - as.numeric(food2bymm$Food_code)
  table(food2byloop$diff)
  
  identical(food2byloop$Food_code, food2bymm$Food_code)
  identical(food2byloop$DR2IGRMS, food2bymm$DR2IGRMS)
######### CHECKING DONE ####################################################
...
##################################################################################################################################    
##################################################################################################################################    
# Code to use a loop.. takes so long. To be deleted in Version 2. 
##################################################################################################################################    
##################################################################################################################################    
...
##################################################################################################################################    
##################################################################################################################################    
# To be deleted up to here in Version 2. 
##################################################################################################################################    
##################################################################################################################################        
