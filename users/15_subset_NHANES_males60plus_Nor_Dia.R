...
# Then select only for males in their 60 plus (over) that are Normal or Diabetic.
# Version 1
...
# Look at the distribution of GLU_index among the selected subpopulation.
  ...
  glu_2_males60over_ND <- subset(glu_2_males60over, GLU_index == "Normal" | GLU_index == "Diabetic" ) 
  dim(glu_2_males60over_ND)
  # Should be 133 rows (people).
  
# Save the glu_2_males60over as a txt file.
  write.table(glu_2_males60over_ND, "Laboratory_data/QCtotal_d_glu_body_meta_demo_males60over_ND.txt", 
              sep="\t", row.names = F, quote = F)
  
# ----------------------------------------------------------------------------------------------------------------  
# Look at the BMI frequency of each group.
# This uses lighter colors for the subpopulation.
  males60over_ND_BMIfreq <- ggplot(data=glu_2_males60over_ND, aes(x=BMXBMI, group=GLU_index, fill=GLU_index)) +
    geom_density(adjust=1.5, alpha=0.4) + space_axes + no_grid +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    labs(x="BMI", y="Density")
  males60over_ND_BMIfreq

  # Save the chart as .pdf.
  ggsave("Laboratory_data/males60over_ND_BMI_by_GLU_index.pdf", 
         males60over_ND_BMIfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Body weight
    # Make sure the labels in the legend are correct. 
  males60over_ND_weightfreq <- ggplot(data=glu_2_males60over_ND, aes(x=BMXWT, group=GLU_index, fill=GLU_index)) +
    geom_density(adjust=1.5, alpha=.4) + space_axes + no_grid +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    labs(x="Body weight (kg)", y="Density") 
  males60over_ND_weightfreq
  
  ggsave("Laboratory_data/males60over_ND_weight_by_GLU_index.pdf", 
         males60over_ND_weightfreq, device="pdf", width=5.3, height=4.5)

# ----------------------------------------------------------------------------------------------------------------  
# Look at the KCAL frequency of each group.   
# Make sure the labels in the legend are correct. 
  males60over_ND_KCALfreq <- ggplot(data=glu_2_males60over_ND, aes(x=KCAL, group=GLU_index, color=GLU_index)) +
    ...
    labs(x="KCAL", y="Density") +
    scale_y_continuous(labels= function(x) format(x, scientific = FALSE))
  males60over_ND_KCALfreq
  
  # Save the chart as .pdf.
  ggsave("Laboratory_data/males60over_ND_KCAL_by_GLU_index.pdf", 
         males60over_ND_KCALfreq, device="pdf", width=5.3, height=4.5)
  
# ----------------------------------------------------------------------------------------------------------------  
# Create a boxplot of KCAL of each GLU_index group.
  males60over_ND_KCAL <- ggplot(glu_2_males60over_ND, aes(x=GLU_index, y=KCAL, fill=GLU_index)) +
    geom_boxplot(outlier.shape = NA) + no_grid + space_axes +
    scale_fill_manual(values= c("aquamarine2", "lightgoldenrod1", "lightpink1") ) +
    geom_jitter(width=0.3)
  males60over_ND_KCAL
  
  ggsave("Laboratory_data/males60over_ND_KCAL_by_GLU_index_box.pdf", 
         males60over_ND_KCAL, device="pdf", width=5.3, height=4.5)
...
# Remove samples (rows) that have missing data in BMXBMI
  df <- glu_2_males60over_ND[complete.cases(glu_2_males60over_ND$BMXBMI), ]
...
# KCAL
  # Remove samples (rows) that have missing data in BMXBMI
  df <- glu_2_males60over_ND[complete.cases(glu_2_males60over_ND$KCAL), ]
