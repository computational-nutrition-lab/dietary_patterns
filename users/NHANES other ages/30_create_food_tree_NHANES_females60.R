# ===============================================================================================================
# Generate food tree out of GLU - FEmales in their 60s
...
# Save. This will be the input in the following procedures.
  write.table(sel.food.record, "Food_D12_FC_cc_f_fems60s.txt", 
...
# so that the data will be formatted in a compatible way to create food tree.
  FilterDbByDietRecords(food_database_fn = "../../Food_tree_eg/NHANESDatabase.txt", 
                        food_records_fn  = "Food_D12_FC_cc_f_fems60s.txt",   # output of FormatFoods.  
                        output_fn =        "Food_D12_FC_cc_f_fems60s_red.txt")
...
# Check if there is any food item reported by people but are missing in the database. 
  check.db(food_database_fn = "../../Food_tree_eg/NHANESDatabase.txt", 
           food_records_fn =  "Food_D12_FC_cc_f_fems60s_red.txt", 
           output_fn =        "Food_D12_FC_cc_f_fems60s_red_missing.txt")

# Load the output and check if the output contains anything? 
  mmm = read.table("Food_D12_FC_cc_f_fems60s_red_missing.txt", sep="\t", header=T)
  head(mmm)
...
# "NodeLabelsMCT.txt" has a list of food levels and names, which comes with this package.
  MakeFoodTree(nodes_fn="../../Food_tree_eg/NodeLabelsMCT.txt", 
               addl_foods_fn = NULL,
               num.levels = 4,
               food_database_fn =   "Food_D12_FC_cc_f_fems60s_red.txt",  
               output_tree_fn =     "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.nwk", 
               output_taxonomy_fn = "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.taxonomy.txt"
  ) 
...
# Make the standard food otu table with data in gram weights of food.
  MakeFoodOtu(food_records_fn=  "Food_D12_FC_cc_f_fems60s.txt",  # need to supply data that have 'FoodAmt' before applying FilterDBByDietRecords.  
              food_record_id =  "SEQN",                          # The ID of your participants
              food_taxonomy_fn= "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.taxonomy.txt",  # Your taxonomy file produced by MakeFoodTree.
              output_fn =       "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.food.otu.txt")  # Output otu file to be saved.

# Make a food otu table with data in grams of fiber per food
  MakeFiberOtu(food_records_fn=  "Food_D12_FC_cc_f_fems60s.txt", 
               food_record_id=   "SEQN", 
               food_taxonomy_fn= "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.taxonomy.txt", 
               output_fn=        "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.fiber.otu.txt")

# Make a food otu table as dehydrated grams per kcal
  MakeDhydrtOtu(food_records_fn=  "Food_D12_FC_cc_f_fems60s.txt", 
                food_record_id =  "SEQN", 
                food_taxonomy_fn= "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.taxonomy.txt", 
                output_fn =       "Foodtree/Food_D12_FC_cc_f_fems60s_red_Lv4.dhydrt.otu.txt")  
