# ===============================================================================================================
# Function to prepare NHNAES data (made into a phyloseq object) pairwise adonis. 
# Version 1
...
# PairwiseAdonis <- function(x, factors, sim.method, p.adjust.m ='bonferroni', perm)
# {
#   library(vegan)
...
#     ad = adonis(x[factors %in% c(co[1,elem],co[2,elem]),] ~ factors[factors %in% c(co[1,elem],co[2,elem])] , method =sim.method, permutations = perm);
...
# PrepForPairwiseAdonis <- function(food.otu.table, demog.table){
#   
...
#   indTF <- demog.table[, 'SEQN'] %in% individuals_v_num
#   
#   # Pick up only those with TRUE (rows with SEQN in individuals_num)
#   demog_sel <<- demog.table[indTF, ]
#   
#   # Output
#   return(paste('The output is saved as "demog_sel".'))
#   
# }
...
# 
# pairwise.adonis(dist_matrix_u, demog_glu_sel$GLU_index )
# 
# 
