...
# ========================================================================================  
 ...
 ########## Need to make it into a function #############
  ...
  # Subset necessary columns.
  Items_raw3 <- subset(Items_raw, select = c(UserName, 
                                             RecallNo, 
                                             FoodAmt_Index_No:A_DRINKS_Index_No))
  ...
  for(i in 1:length(myvar)){
    if(i==1){
      ...
      results[[i]] <- restable
      New_Totals <<- restable
      # restable_sub <- restable[, c(4, 3)]  
      # results[[i]] <- restable_sub 
      # New_Totals <<- restable_sub
    }else if(i>1){
      subsetted <- Items_raw3[, c('UserName','RecallNo', myvar[i])]
      restable <- aggregate(subsetted[, 3] ~ subsetted[, 1] + subsetted[, 2], 
                            data=subsetted, FUN = sum)
      colnames(restable) <- c('UserName', 'RecallNo', paste(myvar[i]))
      restable$User_Day <- paste(restable$UserName, restable$RecallNo, sep = "_")
      restable_sub <- restable[, c(4, 3)]  # take only User_Day_OccNo and means.
      results[[i]] <- restable_sub 
...
# Protein 
  ...
  PROTOutliers <- function(data = New_Totals, min = 10, max = 240){
    PROT_outlier_rows <- subset(data, PROT < min | PROT > max)
    ...
    if(nrow(PROT_outlier_rows) == 0){ cat("\n")}
            ...
            answer <- askYesNo("Remove?")
            if(answer==T){
              data <- subset(data, PROT >= min & PROT <= max)
            }
        }
  }
# ---------------------------------------------------------------------------------------------------------------
...
# Fat 
  ...
  TFATOutliers <- function(data = New_Totals, min = 15, max = 230){
    TFAT_outlier_rows <- subset(data, TFAT < min | TFAT > max)
    ...
    if(nrow(TFAT_outlier_rows) == 0){ cat("\n")}
          ...
          answer <- askYesNo("Remove?")
          if(answer==T){
            data <- subset(data, TFAT >= min & TFAT <= max)
      }
    }
  }
# ---------------------------------------------------------------------------------------------------------------
...
# Vitamin C 
  ...
  VCOutliers <- function(data = New_Totals, min = 5, max = 400){
    VC_outlier_rows <- subset(data, VC < min | VC > max)
    ...
    if(nrow(VC_outlier_rows) == 0){ cat("\n")}
      ...
      answer <- askYesNo("Remove?")
      if(answer==T){
        data <- subset(data, VC >= min & VC <= max)
      }
    }
  }
# ---------------------------------------------------------------------------------------------------------------
...
# Beta-carotene 
  ...
  BCAROutliers <- function(data = New_Totals, min = 15, max = 8200){
    BCAR_outlier_rows <- subset(data, BCAR < min | BCAR > max)
    ...
    if(nrow(BCAR_outlier_rows) == 0){ cat("\n")}
      ...
      answer <- askYesNo("Remove?")
      if(answer==T){
        data <- subset(data, BCAR >= min & BCAR <= max)
      }
    }
  }
